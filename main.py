...
def enchipher_click():
    if combobox.get() == "Caesar's cipher":
        ...
        text = functions.enchip(entry_phrase_enchip.get(), entry_key_enchip.get())
        result = f"Result: \n{text}"
        ...
        text = functions.vigenere(entry_phrase_enchip.get(), entry_key_enchip.get())
        result = f"Result: \n{text}"
        textbox_enchip_result.delete(1.0, "end")
        textbox_enchip_result.insert(1.0, result)
...
def dechipher_click():
    if combobox.get() == "Caesar's cipher":
        text = functions.dechip(entry_phrase_dechip.get(), entry_key_dechip.get())
        result = f"Result: \n{text}"
        ...
        text = functions.de_key_vigenere(entry_phrase_dechip.get(), entry_key_dechip.get())
        result = f"Result: \n{text}"
        textbox_dechip_result.delete(1.0, "end")
        textbox_dechip_result.insert(1.0, result)
...
entry_phrase_enchip = customtkinter.CTkEntry(app, placeholder_text="Phrase", font=Font, height=3, width=250)
entry_phrase_enchip.grid(row=1, column=0, padx=20, pady=20)

entry_phrase_dechip = customtkinter.CTkEntry(app, placeholder_text="Phrase", font=Font, height=3, width=250)
...
textbox_enchip_result = customtkinter.CTkTextbox(app, font=Font, height=100, width=300)
textbox_enchip_result.grid(row=4, column=0, padx=20, pady=20)

textbox_dechip_result = customtkinter.CTkTextbox(app, font=Font, height=100, width=300)
textbox_dechip_result.grid(row=4, column=1, padx=20, pady=20)


def combobox_callback(choice):
    # print("combobox dropdown clicked:", choice)
    entry_phrase_enchip.delete(0, "end")
    entry_key_enchip.delete(0, "end")
    textbox_enchip_result.delete("1.0","end")
    entry_phrase_dechip.delete(0, "end")
